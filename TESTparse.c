...
int TESTparse()
{
	...
	case 7: printf("缺少操作数!\n"); break;
	case 8: printf("缺少逗号!\n"); break;
	case 9: printf("do-while结构不完整!\n"); break;
	case 11: printf("缺乏程序终结符#，语法分析无误！\n"); break;
	case 12: printf("程序异常结束！\n"); break;
	}
	fclose(fp);
	return(es);
}
...
int program()
{
	...
	if (es > 0) return(es);
	es = statement_list();
	if (es > 0&&es!=12) return(es);
	if (strcmp(token, "}")) es = 12;
	else if (strcmp(token, "}"))//判断是否'}'
	{
		es = 2;
		/*return(es);*/
	}
	
	if (left != right) return (es=2);
	//new
	fscanf(fp, "%s %s\n", &token, &token1);
	printf("%s %s\n", token, token1);
	if (es == 12) es = 0;
	if (strcmp(token, "#") == 0) return es;
	else //判断是否'}'
	{
		es = 11;
		return(es);
...
int declaration_stat()
{
	...
	while (strcmp(token, ";") != 0) {
		...
		if (strcmp(token, ",")) return (es = 8);	//连续定义缺少逗号 
		fscanf(fp, "%s %s\n", &token, &token1);
		printf("%s %s\n", token, token1);
	}
...
int statement()
{
	...
	if (es == 0 && strcmp(token, "int") == 0) es = declaration_stat();
	// 加了 ; 判断 便于多个;递归 
...
int factor()
{
	...
	else
	{
		...
		if (strcmp(token, "ID") == 0 || strcmp(token, "NUM") == 0 || strcmp(token, "-") == 0) //负操作
		{
...
int do_stat()
{
	...
	fscanf(fp, "%s %s\n", &token, &token1);
	printf("%s %s\n", token, token1);
	if (!(es == 0 && strcmp(token, "while") == 0)) return (es = 9);// while
